      ...
      <motion.div initial={{ opacity: 0, scale: 0.94, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        ...
        <div className="px-6 pt-6 pb-4 bg-gradient-to-br from-primary to-primary-dark flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => addToast('Report PDF downloaded (demo)', 'success', 2400)}
            ...
            <button onClick={() => addToast('Report printed (demo)', 'success', 2400)}
              className="h-9 px-3.5 rounded-xl bg-white/15 text-white text-[11px] font-bold flex items-center gap-1.5 hover:bg-white/25 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-6 admin-scroll">
          {data ? (
              ...
              <div className="rounded-2xl bg-cream p-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
                  {[
                    ...
                    { label: 'Avg Order', value: inr(data.summary.avgOrder) },
                    { label: 'Cancelled', value: data.summary.cancelledOrders },
                  ].map((s) => (
            ...
            <div className="py-16 text-center">
              <div className="w-16 h-16 mx-auto rounded-3xl bg-primary/8 flex items-center justify-center"><FileText className="w-8 h-8 text-primary" /></div>
              <h4 className="mt-4 font-serif-display text-lg font-bold text-dark">Report Generated (Demo)</h4>
              <p className="mt-1.5 text-sm text-dark/45 font-light">This report is ready for download. In production, the full report data would appear here.</p>
            </div>
          )}
        </div>
      </motion.div>
          ...
          return (
            <motion.div key={report._id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="mt-3.5 flex items-center gap-2 text-[10px] text-dark/35">
                <Calendar className="w-3 h-3" /> Last generated {new Date(report.lastGenerated).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
              </div>
